...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    {
        sLog->outError("bot_ai::SetBotOwner(): bot %s (id: %u) has master %s while trying to set to %s...",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    if (!IAmFree())
    {
        sLog->outError("bot_ai::SetBotOwner(): minion bot %s (id: %u) IS NOT FREE (has master %s) while trying to set to %s",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    ...
    {
        //sLog->outError("bot_ai::SetBotOwner(): player %s (%u) can't add bot %s (FATAL), removing...",
        ...
        if (_ownerGuid)
        {
            sLog->outError("bot_ai::SetBotOwner(): %s's master %s (guid: %u) is found but bot failed to set owner (fatal)! Unbinding bot temporarily (until server restart)...",
...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
    ...
    //    {
    //        //sLog->outError("bot_ai::CheckBotCast(): %s returns SPELL_FAILED_LOWLEVEL", spellInfo->SpellName[0]);
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_DARK_RANGER:
            break;
        default:
            sLog->outError("CheckBotCast(): Unknown bot class %u", _botclass);
            break;
    }
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    {
        sLog->outError("bot_ai::doCast(): %s (bot class %u) tried to cast spell %u on a dead target %s",
        ...
        !(flags & TRIGGERED_FULL_MASK) && (m_botSpellInfo->IsChanneled() || m_botSpellInfo->CalcCastTime()))
    {
        ...
        //    sLog->outError("bot_ai::doCast(): spell %u interrupts movement", spellId);
        me->BotStopMovement();
    }
    ...
    //debug
    //sLog->outError("CheckBotCast(): %s (%u) by %s on %s", m_botSpellInfo->SpellName[0], spellId, me->GetName().c_str(), victim->GetName().c_str());
...
void bot_ai::RezGroup(uint32 REZZ)
{
    ...
    //sLog->outError("RezGroup by %s", me->GetName().c_str());
...
void bot_ai::CureGroup(uint32 cureSpell, uint32 diff)
{
    ...
    //sLog->outError("%s: CureGroup() on %s", me->GetName().c_str(), pTarget->GetName().c_str());
    std::list<Unit*> targets;
    Group const* pGroup = master->GetGroup();
    BotMap const* map;
    Unit* u;
...
bool bot_ai::CanAffectVictim(uint32 schoolMask) const
{
    ...
    {
        sLog->outError("bot_ai::CanDamageVictim(): schoolMask is not present (class = %u)", _botclass);
        return false;
    }
...
void bot_ai::SetStats(bool force)
{
    ...
    /*sLog->outError("*etStats(): Updating bot %s, class: %u, race: %u, level %u, master: %s",
    ...
    switch (myclass)
    {
        ...
        default:
            sLog->outError("minion_ai: *etStats():Init - unknown bot class %u, real class: %u, _botclass: %u", myclass, GetPlayerClass(), _botclass);
            break;
    }
    ...
    switch (myclass)
    {
        ...
        default:
            sLog->outError("_MeleeDamageUpdate(): NIY myclass %u!", uint32(myclass));
            strmult = 0.f; agimult = 0.f; break;
    }
    ...
    if (IsCastingClass(_botclass))
    {
        ...
        if (_botclass == BOT_CLASS_SPHYNX)
        {
            ...
            for (uint8 i = BOT_SLOT_MAINHAND; i <= BOT_SLOT_OFFHAND; ++i)
                if (ItemTemplate const* proto = _equips[i] ? _equips[i]->GetTemplate() : NULL)
...
void bot_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 10)
    {
        sLog->outError("bot_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: %u, entry: %u)",
            int32(count), me->GetName().c_str(), uint32(_botclass), me->GetEntry());
        return;
    }
    if (!spellId)
    {
        sLog->outError("bot_ai::RefreshAura(): spellId is 0 for bot %s (botclass: %u, entry: %u)",
    ...
    if (!spellInfo)
    {
        sLog->outError("bot_ai::RefreshAura(): Invalid spellInfo for spell %u! Bot - %s (botclass: %u, entry: %u)",
...
Unit* bot_ai::_getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    //sLog->outError("bot_ai::getTarget(): bot: %s", me->GetName().c_str());
    ...
    if (!u && !IAmFree() && master->IsInCombat() && Rand() < 15)
    {
        for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_MAX_SPELL; ++i)
        {
            if (Spell const* spell = master->GetCurrentSpell(CurrentSpellTypes(i)))
            {
                ...
                {
                    Unit* victim = ObjectAccessor::FindUnit(spell->m_targets.GetUnitTargetGUID());
    ...
    {
        //sLog->outError("bot %s continues attack common target %s", me->GetName().c_str(), u->GetName().c_str());
        return u;//forced
    }
    //Follow if...
    uint8 followdist = IAmFree() ? BotMgr::GetBotFollowDistDefault() : master->GetBotMgr()->GetBotFollowDist();
    ...
    {
        //sLog->outError("bot %s cannot attack target %s, too far away or not in LoS", me->GetName().c_str(), mytar ? mytar->GetName().c_str() : "");
        return NULL;
    }

    if (u && !IAmFree() && (master->IsInCombat() || u->IsInCombat())/* && !InDuel(u)*/ && !IsInBotParty(u) && (_botPvP || !u->IsControlledByPlayer()))
    {
        //sLog->outError("bot %s starts attack master's target %s", me->GetName().c_str(), u->GetName().c_str());
        return u;
    }
    ...
    {
        //sLog->outError("bot %s continues attack its target %s", me->GetName().c_str(), mytar->GetName().c_str());
    ...
    if (!IAmFree())
    {
        ...
        if (!gr)
        {
            ...
            for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
            {
                ...
                {
                    //sLog->outError("bot %s hooked %s's victim %s", me->GetName().c_str(), bot->GetName().c_str(), u->GetName().c_str());
                    return u;
                }
            }
        }
        else
        {
            ...
            for (GroupReference const* ref = gr->GetFirstMember(); ref != NULL; ref = ref->next())
            {
                ...
                if (u && pl != master &&
                    ...
                    CanBotAttack(u, byspell))
                {
                    //sLog->outError("bot %s hooked %s's victim %s", me->GetName().c_str(), pl->GetName().c_str(), u->GetName().c_str());
                    return u;
                }
    ...
    //if (t)
    //    sLog->outError("bot %s has found new target %s", me->GetName().c_str(), t->GetName().c_str());
...
bool bot_ai::CheckAttackTarget()
{
    ...
    switch (_botclass)
    {
        case BOT_CLASS_DRUID:
            switch (GetBotStance())
            {
                ...
                case BOT_STANCE_NONE:
                    ...
                    break;
                default:
                    sLog->outError("bot_ai::CheckAttackTarget(): druid has NYI bot stance %u", uint32(GetBotStance()));
                    break;
            }
            break;
        case BOT_CLASS_PRIEST:
        ...
        case BOT_CLASS_DARK_RANGER:
            break;
        default:
            sLog->outError("bot_ai: CheckAttackTarget() - unknown bot class %u", _botclass);
            return false;
    }
    ...
    if (!opponent)
    {
        //sLog->outError("bot_ai: CheckAttackTarget() - bot %s lost target", me->GetName().c_str());
        if (me->GetVictim() || me->IsInCombat()/* || !me->getThreatManager().isThreatListEmpty()*/)
        {
            //sLog->outError("bot_ai: CheckAttackTarget() - bot %s Evades", me->GetName().c_str());
            if (me->GetVictim())
                me->AttackStop();
...
void bot_ai::CalculateAttackPos(Unit const* target, Position& pos) const
{
    ...
    for (uint8 i = 0; i != 5; ++i)
    {
        target->GetNearPoint(me, x, y, z, 0.f, dist, Position::NormalizeOrientation(angle));
...
void bot_ai::GetInPosition(bool force, Unit* newtarget, Position* mypos)
{
    ...
    {
        attackpos.m_positionX = newtarget->GetPositionX() - frand(0.5f, 1.5f) * std::cos(me->GetAngle(newtarget));
        attackpos.m_positionY = newtarget->GetPositionY() - frand(0.5f, 1.5f) * std::sin(me->GetAngle(newtarget));
...
void bot_ai::MoveBehind(Unit const* target) const
{
    ...
    {
        float x(0),y(0),z(0);
        target->GetNearPoint(me, x, y, z, me->GetCombatReach(), 0.1f, target->GetAngle(me) + M_PI);
...
WorldObject* bot_ai::GetNearbyRezTarget(float dist) const
{
    ...
    TypeContainerVisitor<acore::WorldObjectListSearcher <NearbyRezTargetCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
...
Unit* bot_ai::FindAffectedTarget(uint32 spellId, uint64 caster, float dist, uint8 hostile) const
{
    ...
    {
        sLog->outError("bot_ai::FindAffectedTarget(): hostile = %u Setting to ALL...", hostile);
        hostile = 255;
    }
...
uint32 bot_ai::InitSpell(Unit const* caster, uint32 spell)
{
    ...
    if (!info)
    {
        sLog->outError("InitSpell(): No SpellInfo found for spell %u", spell);
...
void bot_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        sLog->outError("bot_ai::InitSpellMap(): No SpellInfo found for base spell %u", basespell);
...
void bot_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        sLog->outError("bot_ai::ReleaseSpellCooldown is called for wrong spell %u!", basespell);
        return;
    }
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            switch (myclass)
            {
                ...
                default:
                    sLog->outError("RaceSpellForClass(): unknows race:class combo %u, %u", uint32(myrace), uint32(myclass));
                    return 0;
            }
            break;
        default:
...
void bot_ai::_OnHealthUpdate() const
{
    ...
    //sLog->outError("bot's stats to health add: Stamina (%f), value: %f", stamValue, stamValue * 10.f);
    ...
    //sLog->outError("health to add after stam mod: %i", hp_add);
    uint32 m_totalhp = m_basehp + int32(hp_add); //m_totalhp = uint32(float(m_basehp + hp_add) * stammod);
    //sLog->outError("total base health: %u", m_totalhp);
    ...
    //sLog->outError("overall hp: %u", me->GetMaxHealth());
...
void bot_ai::_OnManaUpdate() const
{
    ...
    //sLog->outError("_OnManaUpdate(): updating bot %s", me->GetName().c_str());
    ...
    //sLog->outError("classinfo base mana = %f", m_basemana);
...
void bot_ai::AdjustTankingPosition() const
{
    ...
    //sLog->outError("AdjustTankingPosition() by %s", me->GetName().c_str());
    ...
    //sLog->outError("AdjustTankingPosition(): atts %u, behind %u", uint32(myattackers.size()), bCount);
    ...
    float const moveDist = -1.f * std::max<float>(opponent->GetObjectSize() * 0.6f, 3.f);
    float moveX;
    float moveY;
    //bool move = false;
    for (uint8 i = 0; i != 3; ++i)
...
bool bot_ai::OnGossipHello(Player* player, uint32 /*option*/)
{
    ...
    if (player->GetGUIDLow() != _ownerGuid)
    {
        if (IAmFree())
        {
...
//                message2 << FCSL("<Insert Coin>");
                message1 << acore::StringFormat(FCSL(u8"你确定要引起 %s 注意?"), me->GetName());
                message2 << FCSL(u8"<塞入硬币>");
...
//                message2 << FCSL("<Try to make an offering>");
                message1 << acore::StringFormat(FCSL(u8"你要引诱 %s?"), me->GetName());
                message2 << FCSL(u8"<尝试提供个机会>");
            }
            else
            {
                message1 << acore::StringFormat(FCSL(u8"你想雇佣 %s吗?"),me->GetName());
//                message2 << FCSL("<Hire bot>");
                message2 << FCSL(u8"<雇佣机器人>") << FCSL("<雇佣机器人>");
            ...
            if (!reason)
            {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, message2.str().c_str(), GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + reason);
            menus = true;
        }
    }
    ...
    if (_ownerGuid)
    {
        ...
        if (player == master)
        {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"管理规则...", GOSSIP_SENDER_ROLES_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"管理阵型...", GOSSIP_SENDER_FORMATION, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"管理天赋...", GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
            if (_botclass < BOT_CLASS_EX_START)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"提供消耗品...", GOSSIP_SENDER_USEITEM, GOSSIP_ACTION_INFO_DEF + 1);
            ...
            if (!gr)
            {
                ...
                if (player->GetNpcBotsCount() > 1)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<创建队伍(all bots)>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else if (!gr->IsMember(me->GetGUID()))
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<添加到队伍>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<添加所有机器人到队伍>", GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
            }
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<移出队伍>", GOSSIP_SENDER_LEAVE_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
        }
        if (player == master || (gr && gr->IsMember(master->GetGUID())))
        {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_MAGE:
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"我需要食物", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"我需要水", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                    if (me->getLevel() >= 70)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"我需要一个茶点桌", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    menus = true;
                    break;
                }
                case BOT_CLASS_ROGUE:
                {
                    ...
                    {
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, acore::StringFormat(FCSL(u8"帮我开个锁 (%u)"), uint32(me->getLevel() * 5)).c_str(), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                        menus = true;
                    }
                    break;
                }
                case BOT_CLASS_WARLOCK:
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"我需要你的复活石", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    if (me->getLevel() >= 68)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"我需要一个灵魂井", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                    menus = true;
                    break;
                }
                default:
        ...
        if (player == master)
        {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_ROGUE:
                {
                    if (me->getLevel() >= 20)
                    {
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"请你更新毒药", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"<选择毒药(主手)>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                        Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                        if (oweap && oweap->GetTemplate()->Class == ITEM_CLASS_WEAPON)
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"<选择毒药(副手)>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                    }
                    break;
                }
                case BOT_CLASS_SHAMAN:
                {
                    if (me->getLevel() >= 10)
                    {
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"请你更新魔法", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"<选择魔法(主手)>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 3);
                        Item const* oweap = _equips[BOT_SLOT_OFFHAND];
                        if (oweap && oweap->GetTemplate()->Class == ITEM_CLASS_WEAPON)
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"<选择魔法(副手)>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 4);
                    }
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"请你去除变形状态", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 5);
                    break;
                }
                case BOT_CLASS_DRUID:
                {
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"请你去除变形状态", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
                    break;
                }
                case BOT_CLASS_HUNTER:
                {
                    if (me->getLevel() >= 10)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<选择宠物类别>", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
            ...
            player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GOSSIP_ICON_TAXI, u8"你被解雇了",
                GOSSIP_SENDER_DISMISS, GOSSIP_ACTION_INFO_DEF + 1, acore::StringFormat(FCSL(u8"你要放弃 %s吗?你可能会后悔的..."),me->GetName()).c_str(), 0, false);

            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"振作起来一点，该死的", GOSSIP_SENDER_TROUBLESHOOTING, GOSSIP_ACTION_INFO_DEF + 1);
        }
    }
    ...
    {
        menus = true;
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"<研究这个生物>", GOSSIP_SENDER_SCAN, GOSSIP_ACTION_INFO_DEF + 1);
    }
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"没什么", 0, GOSSIP_ACTION_INFO_DEF + 1);
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_MAGE:
                {
                    ...
                    if (option == 1 || option == 2) //food, water
                    {
                        ...
                        if (!food)
                        {
                            BotWhisper(acore::StringFormat(FCSL(u8"我还不能制作 %s "), iswater ? "water" : "food").c_str(), player);
                            //player->PlayerTalkClass->ClearMenus();
                    ...
                    else if (option == 3) //refreshment table
                    {
                        ...
                        if (!tableSpellId)
                        {
                            BotWhisper(u8"被禁用", player);
                            break;
                        }
                        if (!IsSpellReady(43987, GetLastDiff(), false))
                        {
                            BotWhisper(u8"还没有准备好", player);
                            break;
                        }
                        ...
                        if (!table->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), tableGOForSpell, me->GetMap(),
                            me->GetPhaseMask(), x, y, z, me->GetOrientation(), G3D::Quat(), 100, GO_STATE_READY))
                        {
                            delete table;
                            BotWhisper(u8"创建桌子失败", player);
                            break;
                        }
                ...
                case BOT_CLASS_ROGUE:
                {
                    ...
                    else if (action == 3)
                    {
                        ...
                        player->ADD_GOSSIP_ITEM(isauto ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<Auto>", GOSSIP_SENDER_CLASS_ACTION2, GOSSIP_ACTION_INFO_DEF + 0);
                    ...
                    else if (action == 4)
                    {
                        ...
                        player->ADD_GOSSIP_ITEM(isauto ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<Auto>", GOSSIP_SENDER_CLASS_ACTION3, GOSSIP_ACTION_INFO_DEF + 0);
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "BACK", 1, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                    ...
                    if (action == 2)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_PET_AVAILABLE_1; i <= BOTAI_MISC_PET_AVAILABLE_11; ++i)
                        {
                            if (uint32 possibleType = GetAIMiscValue(i))
                            {
                                ...
                                player->ADD_GOSSIP_ITEM(same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, name.c_str(), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + possibleType);
                            }
                        }
                        bool noPet = curType == BOT_PET_INVALID;
                        player->ADD_GOSSIP_ITEM(noPet ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<None>", GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + BOT_PET_INVALID);
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "<Auto>", GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + 0);
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "BACK", 1, GOSSIP_ACTION_INFO_DEF + 1);
                    }
                    break;
                }
                case BOT_CLASS_WARLOCK:
                {
                    ...
                    if (action == 2)
                    {
                        ...
                        for (uint32 i = BOTAI_MISC_PET_AVAILABLE_1; i <= BOTAI_MISC_PET_AVAILABLE_5; ++i)
                        {
                            if (uint32 possibleType = GetAIMiscValue(i))
                            {
                                ...
                                ASSERT(cinfo);
                                bool same = possibleType == curType;
                                player->ADD_GOSSIP_ITEM(same ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, cinfo->Name.c_str(), GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + possibleType);
                            }
                        }
                        bool noPet = curType == BOT_PET_INVALID;
                        player->ADD_GOSSIP_ITEM(noPet ? GOSSIP_ICON_BATTLE : GOSSIP_ICON_CHAT, "<None>", GOSSIP_SENDER_CLASS_ACTION4, GOSSIP_ACTION_INFO_DEF + BOT_PET_INVALID);
                    ...
                    else if (action == 1)
                    {
                        ...
                        if (!spellInfo)
                        {
                            BotWhisper(u8"我还不能制造灵魂石!", player);
                            break;
                        }
                    ...
                    else if (action == 3) //soulwell
                    {
                        ...
                        if (!soulwell->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), wellGOForSpell, me->GetMap(),
                            me->GetPhaseMask(), x, y, z, me->GetOrientation(), G3D::Quat(), 100, GO_STATE_READY))
                        {
                            delete soulwell;
                    ...
                    if (!IsCasting())
                    {
                        ...
                        if (result != SPELL_CAST_OK)
                        {
                            ...
                            if (result == SPELL_FAILED_LOW_CASTLEVEL)
                                BotWhisper(u8"我的技能等级还不够", player);
                            else
        ...
        case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
        {
            ...
            //general
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, u8"给我看看你的装备", GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);

            //auto-equip
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"自动装备...", GOSSIP_SENDER_EQUIP_AUTOEQUIP, GOSSIP_ACTION_INFO_DEF + 1);

            //weapons
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"主手武器...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_MAINHAND);
            if (_canUseOffHand())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"副手武器...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_OFFHAND);
            if (_canUseRanged())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"投掷武器...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);
            if (_canUseRelic())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"圣物...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);

            //armor
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"头部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_HEAD);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"肩膀...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_SHOULDERS);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"胸部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_CHEST);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"腰部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_WAIST);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"腿部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_LEGS);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"足部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FEET);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"手腕...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_WRIST);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"手部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_HANDS);
            ...
            if (IsHumanoidClass(_botclass))
            {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"衬衫...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_BODY);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"手指1...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER1);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"手指2...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER2);
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"颈部...", GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_NECK);
            }
            ...
            if (creature->GetCreatureTemplate()->unit_flags2 & UNIT_FLAG2_MIRROR_IMAGE)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Update visual", GOSSIP_SENDER_MODEL_UPDATE, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "BACK", 1, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            //sLog->outError("OnGossipSelect(bot): added %u item(s) to list of %s (requester: %s)",
        ...
        case GOSSIP_SENDER_ROLES_MAIN: //ROLES 1: list
        {
            ...
            for (; role != BOT_MAX_ROLE; role <<= 1)
            {
                ...
                player->ADD_GOSSIP_ITEM(GetRoleIcon(role), GetRoleString(role), GOSSIP_SENDER_ROLES_MAIN_TOGGLE, GOSSIP_ACTION_INFO_DEF + role);
            }
        ...
        case GOSSIP_SENDER_ROLES_GATHERING:
        {
            ...
            for (; role != BOT_MAX_ROLE; role <<= 1)
            {
                ...
                player->ADD_GOSSIP_ITEM(GetRoleIcon(role), GetRoleString(role), GOSSIP_SENDER_ROLES_GATHERING_TOGGLE, GOSSIP_ACTION_INFO_DEF + role);
            }
        ...
        case GOSSIP_SENDER_ABILITIES:
        {
            ...
            if (HasAbilitiesSpecifics())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"技能状态...", GOSSIP_SENDER_ABILITIES_SPECIFICS_LIST, GOSSIP_ACTION_INFO_DEF + 1);
            if (HasAblitiesAllowedList())
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"管理技能...", GOSSIP_SENDER_ABILITIES_USAGE_LIST, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_SENDER_FORMATION:
        {
            ...
            if (HasRole(BOT_ROLE_RANGED))
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, u8"攻击范围...", GOSSIP_SENDER_FORMATION_ATTACK, GOSSIP_ACTION_INFO_DEF + 2);
        ...
        case GOSSIP_SENDER_TROUBLESHOOTING:
        {
            ...
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Fix your power type", GOSSIP_SENDER_TROUBLESHOOTING_FIX, GOSSIP_ACTION_INFO_DEF + 3);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "BACK", 1, GOSSIP_ACTION_INFO_DEF + 4);
            break;
        }
...
bool bot_ai::OnGossipSelectCode(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action, char const* code)
{
    ...
    uint32 gossipTextId;
    if (player->GetGUIDLow() == _ownerGuid || !IAmFree())
    {
        if (_botclass == BOT_CLASS_SPHYNX)
            gossipTextId = GOSSIP_NORMAL_CUSTOM_SPHYNX;
        else if (_botclass == BOT_CLASS_DREADLORD)
...
void bot_ai::OnOwnerDamagedBy(Unit* attacker)
{
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_ARCHMAGE:
            ...
            break;
        default:
            //sLog->outError("minion_ai: OnOwnerDamagedBy() - unknown bot class %u", uint8(_botclass));
            break;
    }
...
bool bot_ai::_equip(uint8 slot, Item* newItem)
{
    ...
    {
        sLog->outError("minion_ai::_equip(): player %s (guidLow: %u) is trying to make bot %s (id: %u) equip item: %s (id: %u, guidLow: %u) which has state ITEM_REMOVED!",
    ...
    if (slot > BOT_SLOT_RANGED || einfo->ItemEntry[slot] != newItemId)
    {
        ...
        if (newItem->GetOwnerGUID() != master->GetGUID() || !master->HasItemCount(newItemId, 1))
        {
            ...
            sLog->outError("minion_ai::_equip(): player %s (guidLow: %u) is trying to make bot %s (id: %u) equip item: %s (id: %u, guidLow: %u) but either does not have this item or does not own it",
                master->GetName().c_str(), master->GetGUIDLow(), me->GetName().c_str(), me->GetEntry(), proto->Name1.c_str(), proto->ItemId, newItem->GetGUIDLow());
            return false;
        }
    ...
    else if (slot == BOT_SLOT_MAINHAND)
    {
        ...
        else if (_equips[BOT_SLOT_OFFHAND] == NULL && einfo->ItemEntry[BOT_SLOT_OFFHAND])
...
bool bot_ai::_resetEquipment(uint8 slot)
{
    ...
    {
        sLog->outError("minion_ai::_resetEquipment(): player %s (guidLow: %u) failed to reset equipment for bot %s (id: %u) in slot %u",
...
void bot_ai::ApplyItemSetBonuses(Item* item, bool apply)
{
    if (item) //(un)equip, NOT from loops
    {
        ...
        for (uint8 i = 0; i != MAX_ITEM_SET_SPELLS; ++i)
        {
            ...
            //sLog->outError("ApplyItemSetBonusesB: %s's %s, %s (%u), %s (%u), icount %u",
    ...
    for (std::list<uint32>::const_iterator itr = itemSets.begin(); itr != itemSets.end(); ++itr)
    {
        ...
        for (uint8 j = 0; j != MAX_ITEM_SET_SPELLS; ++j)
        {
            ...
            //sLog->outError("ApplyItemSetBonusesB (all): %s's %s, %s (%u), %s (%u), c %u, req %u",
...
void bot_ai::ApplyItemsSpells()
{
    ...
    ApplyItemSetBonuses(NULL, true); //item set bonuses
}
...
Item* bot_ai::GetEquipsByGuid(uint64 itemGuid) const
{
    for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
    {
...
void bot_ai::DefaultInit()
{
    ...
    //bot needs to be either directly controlled by player or have pvp flag to be a valid assist target (buffs, heals, etc.)
    me->SetPvP(master->IsPvP());
    me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE);
    if (sWorld->IsFFAPvPRealm())
        me->SetByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP);
    else if (IAmFree())
        me->SetByteValue(UNIT_FIELD_BYTES_2, 1, 0);
...
void bot_ai::ApplyRacials()
{
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            ...
            break;
        default:
            //sLog->outError("bot_ai::ApplyRacePassives(): unknown race %u for bot %s (%u)", uint32(me->getRace()), me->GetName().c_str(), me->GetEntry());
            return;
    }
}
...
void bot_ai::InitEquips()
{
    ...
    else
    {
        do
        {
            ...
            ASSERT(found);
            //ItemTemplate const* proto = item->GetTemplate();
            //sLog->outError("minion_ai::InitEquips(): bot %s (id: %u): found item: for slot %u: %s (id: %u, guidLow: %u)",
...
void bot_ai::_AddItemLink(Player const* forPlayer, Item const* item, std::ostringstream &str, bool addIcon) const
{
    ...
    //sLog->outError("bot_ai::_AddItemLink(): %s", str.str().c_str());
}
//Unused
...
void bot_ai::_LocalizeItem(Player const* forPlayer, std::string &itemName, std::string &suffix, Item const* item) const
{
    ...
    //sLog->outError("bot_ai::_LocalizeItem(): randomPropId = %i", randomPropId);
    if (!randomPropId)
        return;
    ...
    if (randomPropId > 0)
    {
        if (ItemRandomPropertiesEntry const* item_rand = sItemRandomPropertiesStore.LookupEntry(randomPropId))
        {
            ...
            if (suffs)
            {
                ...
                //    sLog->outError("bot_ai::_LocalizeItem(): rand prop suffix for loc %u = %s", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
    else
    {
        if (ItemRandomSuffixEntry const* item_rand = sItemRandomSuffixStore.LookupEntry(-randomPropId))
        {
            ...
            {
                //for (uint8 i = 0; i != MAX_LOCALES; ++i)
                //    sLog->outError("bot_ai::_LocalizeItem(): rand suff suffix for loc %u = %s", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
...
void bot_ai::JustDied(Unit*)
{
    ...
    if (IsTempBot())
    {
        //sLog->outError("Unsummoning temp bot %s (guidLow: %u), owner: %s (guidLow: %u)...",
...
void bot_ai::OnBotSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotSpellMap::iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //sLog->outError("OnBotSpellInterrupted(): Adding cooldown (%u, new: %u) to spell %s (id: %u, schoolmask: %u), reqSchoolMask = %u",
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
        ...
        for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_AUTOREPEAT_SPELL; ++i)
        {
            ...
            if (!info->IsPositive())
            {
                if (!target->IsAlive())
                    ...
                    const_cast<Unit*>(target)->IsImmunedToSpell(info))
    ...
    if (_updateTimerEx1 <= diff && !IAmFree())
    {
        ...
        */
        ...
        {
            //sLog->outError("bot_ai:UpdateEx by %s, found free master, my skills: %u:", me->GetName().c_str(), uint32(_roleMask & BOT_ROLE_MASK_GATHERING));
            ...
            //sLog->outError("bot_ai:UpdateEx: list is %s", woList.empty() ? "empty" : "not empty");
            if (!woList.empty())
            {
                WorldObject* wo = acore::Containers::SelectRandomContainerElement(woList);
                //sLog->outError("bot_ai:UpdateEx: processing %s", wo->GetName().c_str());
                ...
                else
                {
                    ...
                    {
                        Position pos;
                        wo->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 0.f, CONTACT_DISTANCE, wo->GetAngle(me));
    ...
    if (_bootTimer >= 0 && _bootTimer <= int32(diff))
    {
    ...
    if (checkAurasTimer <= diff)
    {
        ...
        {
            //sLog->outError("CheckAuras(): adding %s to group", me->GetName().c_str());
...
uint8 bot_ai::GetPlayerClass() const
{
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            default:
                sLog->outError("GetPlayerClass: %s has unknown Ex bot class %u!", me->GetName().c_str(), _botclass);
                return BOT_CLASS_PALADIN;
        }
    }
    ...
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            case BOT_CLASS_DARK_RANGER:
                return RACE_BLOODELF;
            default:
                sLog->outError("GetPlayerRace: %s has unknown Ex bot class %u!", me->GetName().c_str(), _botclass);
                return RACE_HUMAN;
        }
    }
...
void bot_ai::InitBotCustomSpells()
{
    if (SPELLS_DEFINED)
    {
        //sLog->outError("Bot custom spells initialization... fail...");
        return;
    }
    //sLog->outError("Bot custom spells initialization... success...");
    SPELLS_DEFINED = true;
    ...
    sinfo->CategoryEntry = NULL;
    sinfo->PowerType = POWER_MANA;
    sinfo->CastTimeEntry = NULL;//sSpellCastTimesStore.LookupEntry(2); //250ms
    sinfo->SchoolMask = SPELL_SCHOOL_MASK_SHADOW | SPELL_SCHOOL_MASK_ARCANE;
    sinfo->ExplicitTargetMask = TARGET_FLAG_UNIT;
    ...
    sinfo->AttributesEx4 |= SPELL_ATTR4_NOT_CHECK_SELFCAST_POWER | SPELL_ATTR4_CAN_CAST_WHILE_CASTING;
    ...
    sinfo->AttributesEx4 |= SPELL_ATTR4_DONT_REMOVE_IN_ARENA; //party aura
    ...
    sinfo->Effects[0].DieSides = 0;
    sinfo->Effects[0].BonusMultiplier = 7.f;
    sinfo->Effects[0].DamageMultiplier = 1.f;
    sinfo->Effects[0].RealPointsPerLevel = 15.f;
    sinfo->Effects[0].ValueMultiplier = 1.f;
    ...
    sinfo->Effects[0].RealPointsPerLevel = 0.f;
    sinfo->Effects[0].ValueMultiplier = 1.f;
    sinfo->Effects[0].RadiusEntry = NULL;
    ...
    sinfo->Effects[1].RealPointsPerLevel = 10.f;
    sinfo->Effects[1].ValueMultiplier = 1.f;
    sinfo->Effects[1].RadiusEntry = NULL;
    ...
    //not needed
    //sLog->outString("Re-Loading Spell Proc conditions...");
