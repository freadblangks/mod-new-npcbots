...
public:
    ...
    struct death_knight_botAI : public bot_ai
    {
        ...
        void SpendRunes(SpellInfo const* spellInfo, bool didHit)
        {
        ...
        uint8 GetBotStance() const { return _presence; }
        ...
        void CheckAntiMagic(uint32 diff)
        {
            ...
            u = ObjectAccessor::FindUnit(spell->m_targets.GetObjectTargetGUID());
...
//                (!botPet || me->GetDistance2d(botPet) > sWorld->GetMaxVisibleDistanceOnContinents()))
        ...
        void DoNormalAttack(uint32 diff)
        {
            ...
            AuraEffect const* blop = noDiseases ? NULL : opponent->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DEATHKNIGHT, 0x0, 0x2000000, 0x0, me->GetGUID());
            AuraEffect const* frof = noDiseases ? NULL : opponent->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DEATHKNIGHT, 0x0, 0x4000000, 0x0, me->GetGUID());
            ...
            if (CanAffectVictim(SPELL_SCHOOL_MASK_NORMAL))
            {
                ...
                if (IsSpellReady(DEATH_STRIKE_1, diff) && blop && frof && Rand() < 60 &&
                    GetHealthPCT(me) < (80 - (10*(blop != NULL) + 10*(frof != NULL))) &&
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell)
        {
            ...
            if (baseId == ICY_TOUCH_1 && _presence == DEATH_KNIGHT_FROST_PRESENCE && target->CanHaveThreatList())
            {
                if (SpellThreatEntry const* threatEntry = sSpellMgr->GetSpellThreatEntry(spellId))
                {
                    ...
                    if (baseThreat)
                        target->AddThreat(me, baseThreat * 6, spell->GetSchoolMask(), spell);
                }
            }
        ...
        uint8 GetPetPositionNumber(Creature const* summon) const
        {
            //TODO: garg, aod, drw
            return 0;
        }
        ...
        {
            //sLog->outError("SummonedCreatureDies: %s's %s", me->GetName().c_str(), summon->GetName().c_str());
            //if (summon == botPet)
            //    botPet = NULL;
        }
