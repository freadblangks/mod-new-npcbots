...
public:
    ...
    struct warlock_botAI : public bot_ai
    {
        ...
        void EnterCombat(Unit* u) { canShadowWard = false; bot_ai::EnterCombat(u); }
            ...
            if (!IAmFree() && hasSoulstone && soulstoneTimer <= diff && GetSpell(CREATE_SOULSTONE_1))
            {
                ...
                if (u)
                {
                    ...
                    switch (spellInfo->Id)
                    {
                        ...
                        default:
                            sLog->outError("bot_warlockAI: unknown soulstone Id %u", spellInfo->Id);
...
//                (!botPet || me->GetDistance2d(botPet) > sWorld->GetMaxVisibleDistanceOnContinents()))
        ...
        void ApplyClassSpellCastTimeMods(SpellInfo const* spellInfo, int32& casttime) const
        {
            ...
            if (lvl >= 15 && (baseId == SHADOW_BOLT_1 || baseId == INCINERATE_1))
            {
                ...
                static_cast<bool>(shadowtrance) = (baseId == SHADOW_BOLT_1 && me->HasAura(SHADOW_TRANCE_BUFF));
                ...
                ((spellInfo->SpellFamilyFlags[0] & 0x165) || (spellInfo->SpellFamilyFlags[1] & 0x310C0)))
            {
                ...
                static_cast<bool>(backdraft) = me->HasAura(BACKDRAFT_BUFF) && !(chaoticmind && baseId == SOUL_FIRE_1);
                if (backdraft)
                    pctbonus += 0.3f;
            }
        ...
        void DamageDealt(Unit* victim, uint32& damage, DamageEffectType damageType)
        {
            ...
            if (damage && botPet && me->getLevel() >= 10 && (damageType == SPELL_DIRECT_DAMAGE || damageType == DOT))
            {
                ...
                if (healVal)
                    me->HealBySpell(botPet, sSpellMgr->GetSpellInfo(FEL_SYNERGY_HEAL), healVal);
            }
        ...
        float GetSpellAttackRange(bool longRange) const
        {
...
///*Talent*/lvl >= 50 ? InitSpellMap(UNSTABLE_AFFLICTION_1) : RemoveSpell(UNSTABLE_AFFLICTION_1);
        ...
        void ApplyClassPassives() const
        {
            ...
            RefreshAura(NIGHTFALL, level >= 80 ? 2 : level >= 25 ? 1 : 0);
            ...
            RefreshAura(ERADICATION, level >= 80 ? 2 : level >= 40 ? 1 : 0);
            ...
            RefreshAura(GLYPH_CORRUPTION, level >= 80 ? 3 : level >= 60 ? 2 : level >= 15 ? 1 : 0);
